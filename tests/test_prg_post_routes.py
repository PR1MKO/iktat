...
def setup_case_edit_fail(app, client, monkeypatch):
    ...
    return f'/cases/{case.id}/edit', {'deceased_name': ''}, 'Valami hiba történt. Próbáld újra.'
...
def setup_case_edit_basic_fail(app, client, monkeypatch):
    ...
    def boom():
        raise Exception('fail')
    monkeypatch.setattr(db.session, 'commit', boom)
    return f'/cases/{case.id}/edit_basic', {'deceased_name': ''}, 'Valami hiba történt. Próbáld újra.'
...
def setup_case_documents_fail(app, client, monkeypatch):
    ...
    def boom():
        raise Exception('fail')
    monkeypatch.setattr(db.session, 'commit', boom)
    return f'/cases/{case.id}/documents', {'tox_ordered': '1'}, 'Valami hiba történt. Próbáld újra.'


def setup_add_user_fail(app, client, monkeypatch):
    create_user('admin', 'secret', 'admin')
    login(client, 'admin', 'secret')
    return '/admin/users/add', {'username': 'x', 'password': 'y', 'role': ''}, 'Minden mező kitöltése kötelező.'
...
def setup_edit_user_fail(app, client, monkeypatch):
    ...
    def boom():
        raise Exception('fail')
    monkeypatch.setattr(db.session, 'commit', boom)
    return f'/admin/users/{target.id}/edit', {'username': 'target', 'role': 'iroda'}, 'Valami hiba történt. Próbáld újra.'
