...
from app.models import AuditLog, Case, UploadedFile, db
...
def test_locked_case_blocks_edit_redirects(client, app):
    ...
    resp = client.post(
        f"/cases/{case_id}/edit", data={"deceased_name": "X"}, follow_redirects=True
    )
...
def test_idempotency_tox_doc_double_post_ignored(client, app, monkeypatch, tmp_path):
    ...
    resp1 = client.post(
        f"/cases/{case_id}/generate_tox_doc", data=form, follow_redirects=True
    )
    ...
    with app.app_context():
        files = UploadedFile.query.filter_by(
            case_id=case_id, category="Toxikológiai kirendelő"
        ).all()
        assert len(files) == 1
