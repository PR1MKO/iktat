...
def test_idempotency_certificate_double_post_ignored(client, app, tmp_path, monkeypatch):
    ...
    login(client, 'doc', 'pw')
    resp1 = client.post(f'/ugyeim/{cid}/generate_certificate', data=form, follow_redirects=True)
    assert resp1.status_code == 200
    with app.app_context():
        first = db.session.get(Case, cid).certificate_generated_at
    resp2 = client.post(f'/ugyeim/{cid}/generate_certificate', data=form, follow_redirects=True)
...
def test_assign_pathologist_idempotent_same_assignment(client, app, monkeypatch):
    ...
    form = {'action':'assign','expert_1':'exp','expert_2':'','form_version':version}
    resp1 = client.post(f'/szignal_cases/{cid}/assign', data=form, follow_redirects=True)
    assert resp1.status_code == 200
    with app.app_context():
        version2 = db.session.get(Case, cid).updated_at.isoformat()
    form2 = {'action':'assign','expert_1':'exp','expert_2':'','form_version':version2}
    resp2 = client.post(f'/szignal_cases/{cid}/assign', data=form2, follow_redirects=False)
