...
def test_iroda_can_order_and_szakerto_sees_preselected(client, app, monkeypatch):
    ...
    with app.app_context():
        ...
        logs = (
            ChangeLog.query.filter_by(case_id=cid, field_name="tox_orders")
...
def test_create_and_edit_pages_show_button_for_iroda(client, app):
    ...
    with client:
        ...
        assert f"/ugyeim/{cid}/vizsgalat_elrendelese" not in html_edit
        assert "Vizsgálatok elrendelése" not in html_edit
