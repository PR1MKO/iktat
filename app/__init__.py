...
def create_app(test_config=None):
    ...
    # ✅ SECRET_KEY fix
    flask_app.config["SECRET_KEY"] = os.environ.get("SECRET_KEY") or "dev-secret-key-123"
    ...
    flask_app.config.setdefault(
        'UPLOAD_INVESTIGATIONS_ROOT', Path(flask_app.instance_path) / 'uploads_investigations'
    )
    flask_app.config.setdefault(
    ...
    Path(flask_app.config['UPLOAD_INVESTIGATIONS_ROOT']).mkdir(parents=True, exist_ok=True)
    ...
    flask_app.add_url_rule("/", endpoint="investigations.list_investigations", view_func=_list_investigations)
